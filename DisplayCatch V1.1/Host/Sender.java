...
public class Sender {
    ...
    private static JComboBox<String> positionCombo;
    ...
    private static void createGUI() {
        ...
        positionCombo = new JComboBox<>(new String[]{"BOTTOM", "LEFT", "RIGHT"});
        positionCombo.setPreferredSize(new Dimension(110, 28));
        positionCombo.setToolTipText("Posizione messaggio sul client");
        ...
        bottomPanel.add(positionCombo, BorderLayout.WEST);
    ...
    private static void sendMessage() {
        ...
        // Prefix message with selected position so Listener can place it
        String pos = "BOTTOM";
        if (positionCombo != null && positionCombo.getSelectedItem() != null) pos = positionCombo.getSelectedItem().toString();
        // send position and message on a single line using '|' as delimiter
        String payload = "POS:" + pos + "|" + text;
        ...
        if (sel != null && !sel.isEmpty()) {
            for (String id : sel) server.inviaMsgA(id, payload);
            appendLog("➤ [A selezionati (" + sel.size() + ")] (" + pos + ") " + text);
        } else {
            int count = server.getClientCount();
            server.inviaMsg(payload);
            appendLog("➤ [A tutti (" + count + " dispositivi)] (" + pos + ") " + text);
        }
